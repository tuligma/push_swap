...
/*   Updated: 2024/08/09 22:29:27 by npentini         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	insertion_position_max_min(t_list *current,
	int content, int max, int min)
{
	if (current->next != NULL && (content > max || content < min))
	{
		if (content < *((int *)current->content)
			&& (*((int *)current->content) == max
				|| *((int *)current->next->content) == min))
			return (1);
		else if (content > *((int *)current->content)
			&& (*((int *)current->content) == max
				|| *((int *)current->next->content) == min))
			return (1);
	}
...
{
	if (current->next != NULL && content == data->max
		&& *((int *)current->content) == max)
		return (1);
	else if (content == data->min
		&& *((int *)current->content) == max)
		return (1);
	else if (current->next != NULL
		&& content > *((int *)current->content)
		&& content < *((int *)current->next->content))
		return (1);
	return (-1);
}
...
int	find_insert_position(t_ps_hub *data, t_ps_stack *a, int content)
{
	...
	while (current != NULL)
	{
		...
		if (insertion_position_inbetween(data, current, content, max) == 1)
			return (++position);
		else if (data->a->tail != NULL
			&& current == data->a->tail
			&& content > *((int *)current->content)
			&& content < *((int *)data->a->head->content))
			return (0);
		current = current->next;
		position++;
	}
